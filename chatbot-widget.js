(function() {
    ...
    function init() {
        ...
        var visitorLanguage = ((Array.isArray(navigator.languages) && navigator.languages[0]) || navigator.language || config.siteLanguage || 'en-US').replace('_', '-');
        ...
        function showPrechatError(message) {
            var errorEl = prechatForm ? prechatForm.querySelector('.ai-chat-form-error') : null;
            if (!errorEl) return;
        ...
        function request(path, payload) {
            ...
            return fetch(restBase + path, {
                ...
                return res.json().catch(function() { return {}; }).then(function(data) {
                    if (!res.ok) {
                        var message = data && data.message ? data.message : 'Request failed. Please try again.';
        ...
        function sendMessage() {
            ...
            request('/message', { session_id: sessionId, message: text, visitor_language: visitorLanguage })
                .then(function(data) {
                    removeTyping();
                    isTyping = false;
                    ...
                    if (data && data.response !== undefined) {
                        ...
                        addMessage('assistant', 'The assistant returned an empty response. Please try again.', true);
                    }
                })
                .catch(function(error) {
                    ...
                    isTyping = false;
                    setSendingState(false);
                    if (error && error.code === 'ai_chat_invalid_session') {
                        resetSession();
                        showPrechatError('Your previous chat session expired. Please enter your details to start a new conversation.');
                        return;
                    }
                    inputEl.focus();
                    addMessage('assistant', error && error.message ? error.message : 'Unable to connect right now. Please try again.', true);
                });
        }
        ...
        function initVoice() {
            ...
            recognition.lang = visitorLanguage;
        ...
        function toggleVoice(e) {
            ...
            if (location.protocol !== 'https:' && location.hostname !== 'localhost') {
                addMessage('assistant', 'Voice input requires HTTPS. You can continue by typing your message.', true);
                return;
            }
        ...
        prechatForm.addEventListener('submit', function(e) {
            ...
            request('/session', { name: name, email: email, phone: phone })
                ...
                .catch(function(error) {
                    showPrechatError(error && error.message ? error.message : 'Unable to start the chat. Please try again.');
                })
                .finally(function() {
