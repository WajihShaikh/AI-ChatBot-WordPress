jQuery(document).ready(function($) {
    ...
    function getVisitorLanguage() {
        ...
        return (languages[0] || navigator.language || aiChat.site_language || 'en-US').replace('_', '-');
    }

    function showPrechatError(message) {
        const error = $('.ai-chat-form-error');
        if (!message) {
    ...
    function recoverExpiredSession(payload) {
        if (!payload || payload.code !== 'ai_chat_invalid_session') return false;
        clearStoredSession();
        $('#ai-chat-messages').empty();
        setPrechatVisible(true);
        showPrechatError('Your previous chat session expired. Please enter your details to start a new conversation.');
        return true;
    }

    function setSendingState(sending) {
        $('#ai-chat-send').prop('disabled', sending).toggleClass('is-loading', sending);
        $('#ai-chat-input').prop('disabled', sending);
    }
  ...
  // Form Submission
    $('#ai-chat-user-form').submit(function(e) {
        ...
        // 2. Validate required fields with accessible inline feedback.
        if (!name || !email) {
            showPrechatError('Please enter your name and email address.');
            return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            showPrechatError('Please enter a valid email address.');
            return;
        }

        // Allow common international phone formatting without accepting arbitrary text.
        const phoneRegex = /^[0-9+()\s.\-]{6,25}$/;
        if (phone && !phoneRegex.test(phone)) {
            showPrechatError('Please enter a valid phone number.');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showPrechatError(responseMessage(response.data, 'Unable to start the chat. Please try again.'));
                    $('#ai-chat-user-form button').text('Start Chat').prop('disabled', false);
                }
            },
            error: function(xhr) {
                const payload = xhr.responseJSON && xhr.responseJSON.data ? xhr.responseJSON.data : null;
                showPrechatError(responseMessage(payload, 'Unable to connect. Please try again.'));
                $('#ai-chat-user-form button').text('Start Chat').prop('disabled', false);
            }
        });
    });
    ...
    function initVoice() {
        ...
        recognition.continuous = false; recognition.interimResults = true; recognition.lang = getVisitorLanguage();
    ...
    function toggleVoice(e) {
        e.preventDefault();
        if (location.protocol !== 'https:' && location.hostname !== 'localhost') { addMessage('assistant', 'Voice input requires HTTPS. You can continue by typing your message.'); return; }
...
// Close the window without deleting the visitor's saved conversation.
$('.ai-chat-close-chat').click(function() {
    resetMinimize();
    $('#ai-chat-window').removeClass('active').fadeOut(150, function() {
        $('#ai-chat-button').fadeIn(150);
    });
});
    ...
    function sendMessage() {
        ...
        $.ajax({
            ...
            data: { action: 'ai_chat_send_message', nonce: aiChat.nonce, session_id: sessionId, message: message, visitor_language: getVisitorLanguage() },
            success: function(res) { 
                ...
                } else if (!recoverExpiredSession(res.data)) {
                    addMessage('assistant', responseMessage(res.data, 'The assistant is temporarily unavailable. Please try again.'));
                }
            },
            error: function(xhr) { 
                removeTyping(); 
                ...
                $('#ai-chat-input').trigger('focus');
                const payload = xhr.responseJSON && xhr.responseJSON.data ? xhr.responseJSON.data : null;
                if (!recoverExpiredSession(payload)) {
                    addMessage('assistant', responseMessage(payload, 'Unable to connect right now. Please try again.'));
                }
            }
        });
    }
